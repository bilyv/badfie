...
export function TubelightNavbar({ items, className, value, onValueChange }: NavBarProps) {
  ...
  return (
    ...
    >
      <div className="flex items-center gap-1 sm:gap-2 bg-background/70 border border-border backdrop-blur-lg py-1 px-1 rounded-full shadow-lg">
        {items.map((item) => {
          ...
          return (
            <button
              ...
              className={cn(
                "relative flex-1 cursor-pointer text-xs sm:text-sm font-semibold px-2 sm:px-3 py-1.5 sm:py-2 rounded-full transition-colors",
